...
def resolve_ini_path(path_str: str) -> Path:
    ...
    if p.is_dir():
        ...
        if not candidate.exists():
            raise FileNotFoundError(f"INI folder provided but Master_ini_config.ini not found: {candidate}")
        return candidate
    if p.is_file():
        return p
...
def con_api_response(status: int, body: dict, label: str = ""):
    ...
    if isinstance(body, dict):
        for k, v in list(body.items())[:12]:     # cap noisy responses
            print(f"  │  {k}: {v}")
        if len(body) > 12:
...
def con_run_summary(total: int, success: int, failed: int, skipped: int, duration_s: float):
    print(f"\n{'=' * _W}")
...
def sql_update_control_dynamics(conn, control_id, master_status,
                                doc_no=None, resp_code=None, resp_msg=None, resp_json=None,
                                last_error=None, sent_at=None, resp_at=None):
    sql = """
...
def get_token(out_dir: Path) -> str:
    ...
    save_json(out_dir, "auth_request",
              {"url": D365_TOKEN_URL, "payload": {**data, "client_secret": "***"}})
    ...
    # Show response but mask the token value
...
# =========================================================
# D365 ADD LINE (with date retry)
# =========================================================
def add_line_with_retry(http_headers, out_dir, to_number, line_no,
                        item, qty, base_dt_utc: datetime):
    url      = f"{D365_ODATA_BASE}/TransferOrderLinesV2"
    line_dir = out_dir / "lines"

    attempts = [base_dt_utc.date(), (base_dt_utc + timedelta(days=1)).date()]
    temp = (base_dt_utc + timedelta(days=2)).date()
    while len(attempts) < 5:
        if temp.weekday() < 5:
            attempts.append(temp)
        temp = (datetime.combine(temp, datetime.min.time()) + timedelta(days=1)).date()

    for attempt_no, receipt_date in enumerate(attempts, start=1):
        receipt_dt = datetime(receipt_date.year, receipt_date.month, receipt_date.day,
        ...
        if attempt_no == 1:
            con_info(f"  Line {line_no:>3}  |  Item: {item:<20}  Qty: {qty}")
        else:
            con_warn(f"  Line {line_no:>3}  |  Retry attempt {attempt_no}  "
                     f"(receipt date shifted to {receipt_date})")

        con_api_request("POST", url, payload)
        save_json(line_dir, f"line_{line_no}_attempt_{attempt_no}_request",
                  {"url": url, "payload": payload})

        status, body = http_post_json(url, http_headers, payload)

        save_json(line_dir, f"line_{line_no}_attempt_{attempt_no}_response",
                  {"status": status, "body": body})
        con_api_response(status, body, label=f"LINE {line_no} attempt {attempt_no}")
...
def process_one(conn, control_id: int, source_id: str) -> tuple[bool, str, int]:
    ...
    if not header:
        raise RuntimeError(f"No INT.TransferOrderHeader for ControlId={control_id}")

    lines = sql_get_lines(conn, control_id)
    if not lines:
        raise RuntimeError(f"No INT.TransferOrderLine for ControlId={control_id}")

    con_info(f"Header: {header.ShippingWarehouseId} → {header.ReceivingWarehouseId}  "
             f"|  Ship: {header.RequestedShippingDate}  Receipt: {header.RequestedReceiptDate}")
    ...
    header_payload = {
        ...
        "RequestedShippingDate": datetime(requested_ship.year, requested_ship.month,
                                          requested_ship.day, 12, 0, 0,
                                          tzinfo=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "RequestedReceiptDate":  datetime(requested_recv.year, requested_recv.month,
                                          requested_recv.day, 12, 0, 0,
                                          tzinfo=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
    }
    if D365_COMPANY:
    ...
    try:
        ...
        resp = str(ex)
        sql_update_control_dynamics(conn, control_id, master_status="ERROR",
    ...
    for ln in lines:
        ...
        if ok:
            ...
            all_ok       = False
            last_failure = f"Line {ln.LineNo} failed: HTTP {http_status} | {body}"
            break
    ...
    if not all_ok:
        ...
        con_order_summary(source_id, to_number, lines_sent,
                          success=False, duration_s=duration, error=last_failure)
        raise RuntimeError(last_failure)

    # Success
    sql_update_control_dynamics(conn, control_id, master_status="SENT_DYNAMICS",
                                doc_no=to_number,
                                resp_code="OK",
...
def main():
    ...
    try:
        ...
        for idx, row in enumerate(pending, start=1):
            control_id, source_id = row.TransferControlId, row.SourceID
